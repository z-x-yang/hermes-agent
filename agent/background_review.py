...
_MEMORY_REVIEW_PROMPT = (
    ...
    "Save only facts that should influence turns even when no particular skill is loaded: "
    "identity, relationships, cross-domain preferences or corrections, and stable "
    "cross-domain environment facts.\n"
    "If a lesson only matters when a specific skill's trigger matches, do not save it to "
    "memory; it belongs in that skill. Project-specific changing truth belongs in its "
    "canonical project source, and temporary state belongs in ledger/todo/session.\n\n"
    "Use the memory tool only when something qualifies. If nothing does, say "
    "'Nothing to save.' and stop; that is a normal result."
)
...
_EDIT_DISCIPLINE = (
    ...
    "pointer is dead weight. But quality-critical every-run content — "
    "the output templates, skeletons, and MUST-rules that shape what "
    "every run produces — stays in the always-loaded body; references/ "
    "load is discretionary and a budget-pressured run may skip it, so "
    "never push the thing that guarantees output quality behind a "
    "pointer.\n"
    "  • PRESERVE SUBSTANCE — sharpen wording, never genericize content. "
    "Concrete templates, worked examples, section/emoji skeletons, typed "
    "guidance ('for type X do Y'), and hard MUST-rules ARE the skill's "
    "value. Never replace them with generic paraphrase ('write good "
    "notes', 'use judgment', 'as needed', 'keep it high-signal') — that "
    "reads as sharpening but is silent quality erosion. Two smells that "
    "your 'sharpening' is actually erosion: (a) a concrete block became "
    "vague prose while byte count barely changed or grew; (b) a skill "
    "written in the user's language drifted toward generic English. Keep "
    "the skill's working language. Specificity is NOT compressible: "
    "merging two or more concrete examples or type-branches into one "
    "general rule counts as substance removal, not sharpening, unless "
    "the user explicitly asked to generalize. Literal template strings — "
    "emoji/section headings, URL or ID formats like 'message://' or "
    "'rfc822msgid' — are copied verbatim, never reworded into "
    "descriptive prose. If unsure whether a block is redundant or "
    "load-bearing, keep it.\n\n"
)

_SKILL_REVIEW_PROMPT = (
    "Review the conversation above and update the skill library only when the session "
    "produced a durable, reusable lesson with a clear class-level owner. Nothing to "
    "save is a normal result; do not manufacture an update to make the review look useful.\n\n"
    ...
    + _EDIT_DISCIPLINE +
    "User-preference placement: when a style, format, workflow, or tool lesson only "
    "matters for this class of task, put it in the governing skill, not memory. A "
    "cross-domain preference that should influence unrelated turns belongs in memory, "
    "not a task skill. Do not duplicate the same lesson across both stores; split only "
    "when there are genuinely distinct cross-domain and skill-specific semantics.\n\n"
    "If you notice two existing skills that overlap, note it in your "
    "reply — the background curator handles consolidation at scale.\n\n"
    "Protected skills (DO NOT edit these):\n"
    "  • Bundled skills (shipped with Hermes, e.g. 'evelyn-agent').\n"
    ...
    "'Nothing to save.' is a normal result when the session produced no durable, "
    "reusable lesson with a clear owner. Otherwise, make the smallest owner-correct update."
)

_COMBINED_REVIEW_PROMPT = (
    "Review the conversation above and update two things:\n\n"
    "**Memory**: facts that should influence turns even when no particular skill is loaded — "
    "identity, relationships, cross-domain preferences or corrections, and stable cross-domain "
    "environment facts. Save only those with the memory tool.\n\n"
    "**Skills**: durable, reusable lessons that matter when a specific skill's trigger matches. "
    "Update only when there is a clear class-level owner. Nothing to save is a normal result; "
    "do not manufacture a skill update.\n\n"
    ...
    "(2), or (3).\n\n"
    + _EDIT_DISCIPLINE +
    "Placement is exclusive by default: a preference that should influence unrelated turns "
    "belongs in memory; a style, format, workflow, or tool lesson that only matters for one "
    "class of task belongs in its governing skill. Do not duplicate the same lesson across "
    "both stores. Split only when there are genuinely distinct cross-domain and skill-specific "
    "semantics. Project-specific changing truth belongs in its canonical project source.\n\n"
    ...
    "troubleshooting skill — never 'this tool does not work' as a "
    "standalone constraint.\n\n"
    "Act only on a dimension with real durable signal and a canonical owner. If nothing "
    "qualifies, say 'Nothing to save.' and stop; that is a normal result."
)
