...
3. P0：为 `Explore` / `Plan` 提供 read-only / no-external-side-effect runtime enforcement；`general-purpose` 对命名外部副作用工具使用封闭 allowlist，但保留可受正常审批约束的原始 shell/process。
...
2. **Fail-closed capability**：`Explore` / `Plan` 的写入与外部副作用由 runtime 硬限制；`general-purpose` 的封闭 allowlist 排除邮件、Notion、cron 等命名副作用工具，但其 `terminal` / `process` 不是无网络、无副作用沙箱，仍按正常 terminal 审批与任务指令治理。
...
can_external_side_effects: true  # raw terminal/process can reach external systems
...
- 命名外部副作用工具默认不在 allowlist：邮件发送、Notion 写入、cron 修改、公开发布等应由 parent 或用户批准的专用 workflow 执行。
- `terminal` 与 `process` 仍保留给 `general-purpose`。原始 shell 可以访问网络、调用外部 CLI 并产生外部副作用，因此该 profile **不是** runtime-hard no-side-effect sandbox；这些行为继续受正常 terminal 审批、安全检查与 task instructions 约束。本设计不新增 sandboxed runner。
...
- `Explore` / `Plan` 不获得 `terminal` / `process`，因此保持 hard read-only / no-external-side-effect。
- `general-purpose` 保留原始 `terminal` / `process`；shell 可以绕过命名工具分类而访问外部系统，所以 runtime metadata 必须标记 `can_external_side_effects: true`，并依赖既有 terminal 审批、安全检查与 task instructions，而不是声称硬隔离。
...
保留 transcript 同时受 TTL、记录数和序列化字节预算约束：

- operator-only 配置 `delegation.max_retained_subagent_bytes` 默认 `16777216`（16 MiB），不出现在 `delegate_task` / `delegate_continue` model-facing schema。
- 单条初始 record 超过预算时 fail closed，不创建可续接 handle。
- 聚合裁剪只删除未 in-flight 的最旧记录；claimed/in-flight record 永不驱逐，也绝不让存储超过预算。
- 如果成功 continuation 的新 transcript 无法装入预算，成功结果照常返回，并附 `retention_dropped` 与明确 note；对应 handle 被永久 invalidated，后续 continue 稳定失败。
...
3. `general-purpose` 默认允许 repo 内文件修改，排除命名外部副作用工具，同时如实标记 raw shell/process 仍可能产生外部副作用。
...
2. External side-effect policy：`Explore` / `Plan` 无法执行外部副作用；`general-purpose` 无法直接调用邮件、Notion、cron 等命名工具，但测试必须承认其 raw shell/process 不是无副作用沙箱。
...
| read-only 靠 prompt 被绕过 | `Explore` / `Plan` 在 resolved tools 层硬限制；`general-purpose` 只声称命名工具 allowlist，不把 raw shell 描述成无副作用隔离 |
