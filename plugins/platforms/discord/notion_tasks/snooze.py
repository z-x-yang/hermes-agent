...
def _safe_script_stem(value: str) -> str:
    ...
    return text[:120] or f"task_snooze_{int(time.time() * 1000)}"


def _snooze_script_content(record: dict) -> str:
    payload = {
        "page_id": str(record.get("page_id") or ""),
        "title": str(record.get("title") or "Task"),
        "next_check": str(record.get("next_check") or ""),
    }
    return textwrap.dedent(f"""\
        # Auto-generated by Hermes Discord /task-snooze. Do not edit by hand.
        import asyncio
        from plugins.platforms.discord.notion_tasks import detection
        from plugins.platforms.discord.notion_tasks.notion_client import NotionClient, _same_notion_minute
        from plugins.platforms.discord.notion_tasks.snooze import reminder_content

        PAYLOAD = {json.dumps(payload, ensure_ascii=False, sort_keys=True)}

        def _date_start(page):
            date_prop = (((page or {{}}).get("properties") or {{}}).get("Next Check") or {{}}).get("date")
            return (date_prop or {{}}).get("start")

        async def main():
            page_id = PAYLOAD["page_id"]
            title = PAYLOAD["title"] or "Task"
            expected_next = PAYLOAD.get("next_check") or ""
            try:
                page = await NotionClient().get_page(page_id)
            except Exception:
                print(reminder_content(title, page_id, status_unknown=True))
                return
            status, _kind = detection.read_status(page)
            if status in {{"Done", "Dropped"}}:
                return
            current_next = _date_start(page)
            if expected_next and current_next and not _same_notion_minute(expected_next, current_next):
                return
            if expected_next and not current_next:
                return
            print(reminder_content(detection.page_title(page) or title, page_id))
...
def create_snooze_cron(record: dict, *, deliver: str, due_at: datetime) -> dict:
    """Create a one-shot no-agent cron job and its generated reminder script."""
    from hermes_constants import get_hermes_home
    from cron.jobs import create_job

    rec_id = str(record.get("id") or f"{record.get('page_id', 'task')}:{int(time.time() * 1000)}")
    script_rel = f"task_snooze/{_safe_script_stem(rec_id)}.py"
    script_path = get_hermes_home() / "scripts" / script_rel
    script_path.parent.mkdir(parents=True, exist_ok=True)
    script_path.write_text(_snooze_script_content(record), encoding="utf-8")
    job = create_job(
        prompt="",
        schedule=due_at.isoformat(),
        name=f"task-snooze:{str(record.get('page_id') or '')[:8]}",
        repeat=1,
        deliver=deliver,
        script=script_rel,
        no_agent=True,
    ...
    try:
        from tools.cronjob_tools import _notify_provider_jobs_changed_safe
        _notify_provider_jobs_changed_safe()
    except Exception:
        logger.debug("notion task snooze: cron provider notify failed", exc_info=True)
    return job
...
class SnoozeStore:
    ...
    def attach_cron(self, rec_id: str, *, job: dict) -> None:
        rec = self._records.get(str(rec_id))
        if not rec:
            return
        rec["cron_job_id"] = str(job.get("id") or "")
        rec["cron_next_run_at"] = str(job.get("next_run_at") or "")
        rec["cron_script"] = str(job.get("script") or "")
    ...
    def pending_for(self, page_id: str) -> dict | None:
        ...
        pid = str(page_id).replace("-", "").lower()
        candidates = [
            (rec_id, rec) for rec_id, rec in self._records.items()
            if rec.get("page_id") == pid and rec.get("status") in {"pending", "cron_scheduled"}
        ]
        if not candidates:
            return None
        rec_id, _rec = max(candidates, key=lambda kv: float(kv[1].get("updated_at", 0) or 0))
