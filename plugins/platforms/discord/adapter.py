...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Evelyn session")
        ...
        @tree.command(name="status", description="Show Evelyn session status")
        ...
        @tree.command(name="stop", description="Stop the running Evelyn agent")
        ...
        @tree.command(name="update", description="Update Evelyn (powered by Hermes Agent) to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Evelyn gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start an Evelyn session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Evelyn in the thread",
    ...
    def _resolve_thread_parent(self, channel: Any) -> Any:
        """Resolve a thread's parent channel for forum detection.

        Prefers the hydrated ``.parent``; falls back to a client cache lookup
        by ``.parent_id`` when discord.py didn't attach the parent object.
        Returns the channel itself when no parent can be resolved — callers
        treat that as "not a forum" (the safe default for the common case of
        text-channel threads, which always have a cached parent).
        """
        ...
        if parent_id and self._client:
            try:
                resolved = self._client.get_channel(int(parent_id))
            except (TypeError, ValueError):
                resolved = None
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Evelyn: **{name}**"
    ...
    def _derive_thread_name(self, content: str) -> str:
        """Derive a short, human-friendly thread name from a message.

        Strips Discord mention syntax (users / roles / channels) so thread
        titles don't show raw ``<@id>``, ``<@&id>`` or ``<#id>`` markers — the
        ID isn't meaningful to humans glancing at the thread list (#6336) — and
        truncates to Discord's effective display length. Returns ``""`` when
        nothing meaningful remains; callers decide their own fallback.
        """
        content = (content or "").strip()
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        """
        thread_name = self._derive_thread_name(message.content or "") or "Evelyn"
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Evelyn: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Evelyn handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Evelyn could not create a Discord thread for "
