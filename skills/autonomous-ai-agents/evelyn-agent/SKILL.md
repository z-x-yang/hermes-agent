...
- **Single:** `delegate_task(description=..., prompt=..., subagent_type=..., review_root=..., run_in_background=...)`; `review_root` is optional and valid only for a top-level single Reviewer, must be an exact absolute local Git worktree root, and never supports remote/cluster paths.
...
- **Profiles:** `Explore` and `Plan` are read-only one-shot agents; Plan returns implementation-plan research inputs while the parent owns the final plan. `Reviewer` is a fresh-context one-shot code reviewer that accepts an ordinary self-contained prompt and returns an ordinary final response; `general-purpose` handles multi-step execution and is automatically retained only after explicit successful completion. Omission still resolves to `general-purpose`. Reviewer's profile system instructions preload one fixed runtime-owned method covering scope/intent, surrounding call and data/error/cleanup paths, runtime/test wiring and false-greens, plus defect-relevant reuse/simplification/performance; this does not grant dynamic skill discovery.
- **Context:** children receive a runtime Core Contract plus explicit task data and no parent history. A same-provider/endpoint GP additionally inherits the parent's SOUL/MEMORY/USER context; cross-provider/endpoint routes and fallbacks strip it. Explore, Plan, and Reviewer remain lean. GP and Reviewer additionally load repository project context/workspace snapshot. Reviewer requires `read_file`, `search_files`, and `terminal`; readonly web is optional under current parent authority. It has no named Notion, Mail, memory/session, browser, write/patch, process/execute-code, or delegation tools. Raw terminal is not a hard no-write/no-external-side-effect sandbox, so its prompt forbids mutation/private-source access and the controller verifies the worktree afterward.
- **Scheduling:** top-level omission uses the selected profile default (`Reviewer` foreground; others background); a nested omission runs foreground. An explicit `run_in_background` boolean wins where runtime permits it. Background results re-enter the parent conversation on completion.
