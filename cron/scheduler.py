...
# Sequential (env-mutating) cron jobs — no_agent script jobs with a workdir
# still os.chdir() the whole process — must run one at a time, but must NOT
# block the ticker thread.  A persistent single-thread executor preserves
# ordering across ticks while keeping dispatch fire-and-forget, the same as the
# parallel pool.  LLM workdir jobs carry their cwd in a ContextVar (see
# run_job) and dispatch through the normal parallel pool.
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Evelyn — {task_name}"
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    _cron_task_id = _cron_session_id
    _registered_terminal_override = False
    ...
    # time), we pin it in a ContextVar so:
    #   - build_context_files_prompt() picks up AGENTS.md / CLAUDE.md /
    #     .cursorrules from the job's project dir, AND
    #   - terminal/file/code-exec tools run commands from there.
    #
    # This used to mutate os.environ["TERMINAL_CWD"], forcing all workdir
    # cron jobs through one global sequential queue (writer side of
    # _terminal_cwd_lock).  ContextVar cwd keeps LLM cron jobs isolated, so a
    # long paper digest no longer blocks hourly email triage solely because
    # both jobs have a workdir configured.
    _job_workdir = (job.get("workdir") or "").strip() or None
    ...
    try:
        ...
        # Give each workdir cron run its own terminal/code-exec environment.
        # The cwd itself is ContextVar-scoped, but the terminal backend keeps a
        # mutable live env.cwd for `cd` tracking. Sharing the default env across
        # parallel cron jobs would let one job's `cd` leak into another job, so
        # force task isolation for this cron session while preserving cwd.
        if _job_workdir:
            try:
                ...
                register_task_env_overrides(
                    _cron_task_id,
                    {"cwd": _job_workdir, "_force_task_isolation": True},
                )
                _registered_terminal_override = True
            except Exception as _term_override_exc:
                logger.exception(
                    "Job '%s': failed to register terminal workdir override",
                    job_id,
                )
                raise RuntimeError(
                    "failed to isolate cron workdir terminal environment"
                ) from _term_override_exc

        def _run_cron_agent():
            return agent.run_conversation(prompt, task_id=_cron_task_id)

        _cron_future = _cron_pool.submit(_cron_context.run, _run_cron_agent)
...
"""
        ...
        # Clean up task-scoped terminal override and ContextVar
        # session/delivery/cwd state for this job.
        if _registered_terminal_override:
            try:
                from tools.terminal_tool import clear_task_env_overrides

                clear_task_env_overrides(_cron_task_id)
            except Exception:
                pass
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        # Partition due jobs: pure-script no_agent jobs with a workdir still use
        # os.chdir() inside run_job, so keep those serialized. LLM cron jobs now
        # carry workdir via a ContextVar, so they are safe to dispatch through
        # the normal parallel pool and no longer head-of-line block each other.
        sequential_jobs = [j for j in due_jobs if j.get("no_agent") and (j.get("workdir") or "").strip()]
        parallel_jobs = [j for j in due_jobs if not (j.get("no_agent") and (j.get("workdir") or "").strip())]
