...
@pytest.mark.asyncio
async def test_stream_consumer_fallback_sends_tail_after_successful_partial_overflow():
    """Discord edit-overflow can report success=True for visible partials;
    the consumer must still treat partial_overflow as incomplete and send only
    the missing tail."""
    adapter = MagicMock()
    adapter.MAX_MESSAGE_LENGTH = 4096
    adapter.edit_message = AsyncMock(
...
@pytest.mark.asyncio
async def test_fresh_final_partial_send_sends_tail_without_editing_or_repinging():
    """Regression for Discord final-only long replies: if fresh-final send
    lands the head chunks then fails, do not edit-overflow the preview and do
    not send the full final again. Preserve the partial head and send only the
    missing tail."""
    adapter = MagicMock()
    adapter.MAX_MESSAGE_LENGTH = 4096
        ...
        side_effect=[
            SendResult(
                ...
                raw_response={
                    ...
                    "delivered_chunks": 1,
                    "total_chunks": 2,
                    "last_message_id": "partial-head",
                    "delivered_prefix": "hello ",
                    "message_ids": ["partial-head"],
                },
            ),
            SendResult(success=True, message_id="tail-1"),
        ]
    )
    adapter.edit_message = AsyncMock()
    adapter.delete_message = AsyncMock(return_value=True)
    ...
    consumer = GatewayStreamConsumer(
        ...
        initial_reply_to_id="user-msg",
    )
    consumer._message_id = "preview-1"
    consumer._last_sent_text = "preview text"
    ...
    assert consumer._fallback_final_send is True
    assert consumer._fallback_prefix == "hello "
    assert consumer._fallback_preserve_partial_messages is True

    await consumer._send_fallback_final("hello world")

    assert adapter.send.await_count == 2
    tail_call = adapter.send.await_args_list[1].kwargs
    assert tail_call["content"] == "world"
    assert tail_call["reply_to"] is None
    adapter.delete_message.assert_not_awaited()
    assert consumer.final_response_sent is True
    assert consumer.final_content_delivered is True
...
@pytest.mark.asyncio
async def test_first_final_send_partial_sends_tail_without_repinging():
    """Buffer-only final delivery has no preview message id. A partial first
    final send still needs the same tail-only fallback, not a whole-message
    resend."""
    adapter = MagicMock()
    adapter.MAX_MESSAGE_LENGTH = 4096
    adapter.send = AsyncMock(
    ...
    assert adapter.send.await_count == 2
    first_call = adapter.send.await_args_list[0].kwargs
    assert first_call["reply_to"] == "user-msg"
    tail_call = adapter.send.await_args_list[1].kwargs
    assert tail_call["content"] == "world"
    assert tail_call["reply_to"] is None
    assert consumer.final_response_sent is True
