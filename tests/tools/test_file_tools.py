...
def _resolved(path: str) -> str:
    """Expected shell-layer path: write/patch pass the symlink-resolved
    absolute path to file_ops (worktree-cwd fix), so on macOS /tmp/x
    becomes /private/tmp/x while on Linux it is unchanged."""
...
class TestWriteFileHandler:
    ...
    def test_writes_content(self, mock_get):
        ...
        mock_ops.write_file.assert_called_once_with(_resolved("/tmp/out.txt"), "hello world!\n")
...
class TestPatchHandler:
    ...
    def test_replace_mode_calls_patch_replace(self, mock_get):
        ...
        assert result["status"] == "ok"
        mock_ops.patch_replace.assert_called_once_with(_resolved("/tmp/f.py"), "foo", "bar", False)
    ...
    def test_replace_mode_replace_all_flag(self, mock_get):
        ...
        mock_ops.patch_replace.assert_called_once_with(_resolved("/tmp/f.py"), "x", "y", True)
