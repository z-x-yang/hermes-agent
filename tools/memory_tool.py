...
class MemoryStore:
    ...
    def _render_block(self, target: str, entries: List[str]) -> str:
        ...
        if target == "user":
            ...
            header = (
                f"MEMORY (your notes from past sessions — background context; "
                f"may be stale) [{pct}% — {current:,}/{limit:,} chars]"
            )
...
MEMORY_SCHEMA = {
    ...
    "description": (
        ...
        "detail, or you learn a stable fact about their environment. Save only facts that "
        "should influence turns even when no particular skill is loaded. If content only "
        "matters when a named skill applies, patch that skill instead of memory. Project-specific "
        "changing truth belongs in its canonical project source. Do not duplicate the same "
        "lesson across memory and a skill. Priority: cross-domain user preferences and "
        "corrections > cross-domain environment facts. The best memory stops the user "
