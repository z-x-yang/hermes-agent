...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "PLACEMENT: If a durable lesson is only relevant when an existing skill's trigger "
        "matches, patch that skill instead of memory. Memory is for cross-domain facts that "
        "should affect unrelated turns; project-specific changing truth belongs in its "
        "canonical project source. Do not duplicate the same lesson across stores.\n\n"
