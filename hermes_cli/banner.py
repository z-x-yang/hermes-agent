...
[#CD7F32]███████╗ ╚████╔╝ ███████╗███████╗  ██║   ██║ ╚████║[/]
[#CD7F32]╚══════╝  ╚═══╝  ╚══════╝╚══════╝  ╚═╝   ╚═╝  ╚═══╝[/]"""
# Discord-derived Evelyn left-facing near-profile (compact, fits in the left panel).
# The gold moon is the same identity anchor as her Discord avatar hair clip.
EVELYN_PORTRAIT = """[#CD7F32]           ⢀⣀⣠⣤⣤⣄⣀[/]
[#CD7F32]        ⢀⣴⣿⣿⣿⣿⣿⣿⣿⣿⣷[/]
[#FFBF00]        ⡹⢿⢿⡿⡿⢿⡟⠛⢹⣍⣡⣣⣤[/]
[#FFBF00]      ⢠⣿⣷⣶⡾⢳⣷⣾⡟⢸[/][bold #FFD700]☾[/][#FFBF00]⣿⣿⣿⣿⣇[/]
[#FFD700]      ⢾⣿⠿⠟⠓⣂⠻⠵⣿⣶⣾⣿⣿⣿⣿⣿[/]
[#FFD700]       ⢻⡷ ⠈⠋⠙ ⢨⣿⣿⡧⠹⣿⣿⡿[/]
[#FFBF00]       ⢸⡁     ⡸⣿⣿⣏⣴⣿⣿⡇[/]
[#FFBF00]       ⣾⣷      ⣿⣿⣿⣿⣿⡏⣽[/]
...
# Stable internal compatibility alias for extensions that imported the old
# symbol name. The user-visible art itself is Evelyn.
HERMES_CADUCEUS = EVELYN_PORTRAIT
...
def format_banner_version_label() -> str:
    ...
    base = f"Evelyn v{VERSION} ({RELEASE_DATE}) · powered by Hermes Agent"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
                         ...
                         provider: str = None):
    """Build and print a welcome banner with Evelyn on left and info on right.
    ...
    # Use the skin's custom hero art when provided; otherwise show Evelyn.
    try:
        from hermes_cli.skin_engine import get_active_skin
        _bskin = get_active_skin()
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else EVELYN_PORTRAIT
    except Exception:
        _bskin = None
        _hero = EVELYN_PORTRAIT
    left_lines = ["", _hero, ""]
