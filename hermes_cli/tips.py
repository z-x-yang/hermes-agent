...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or evelyn -c.",
    ...
    "Ctrl+Z suspends Evelyn to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "evelyn -c resumes your most recent CLI session. evelyn -c \"project name\" resumes by title.",
    "evelyn -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "evelyn -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "evelyn chat -t web,terminal enables only specific toolsets for a focused session.",
    "evelyn chat -s github-pr-workflow preloads a skill at launch.",
    "evelyn chat -q \"query\" runs a single non-interactive query and exits.",
    "evelyn chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "evelyn chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "evelyn --yolo bypasses all dangerous command approval prompts for the entire session.",
    "evelyn chat --source telegram tags the session for filtering in evelyn sessions list.",
    "evelyn -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "evelyn doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "evelyn dump outputs a compact setup summary — great for bug reports.",
    "evelyn config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "evelyn config edit opens config.yaml in your default editor.",
    "evelyn config check scans for missing or stale configuration options.",
    "evelyn sessions browse opens an interactive session picker with search.",
    "evelyn sessions stats shows session counts by platform and database size.",
    "evelyn sessions prune --older-than 30 cleans up old sessions.",
    "evelyn skills search react --source skills-sh searches the skills.sh public directory.",
    "evelyn skills check scans installed hub skills for upstream updates.",
    "evelyn skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "evelyn skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "evelyn mcp add github --command npx adds MCP servers from the command line.",
    "evelyn mcp serve runs Evelyn itself as an MCP server for other agents.",
    "evelyn auth add lets you add multiple API keys for credential pool rotation.",
    "evelyn completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "evelyn logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "evelyn backup creates a zip backup of your entire Evelyn home directory.",
    "evelyn profile create coder creates an isolated profile that becomes its own command.",
    "evelyn profile create work --clone copies your current config and keys to a new profile.",
    "evelyn update syncs new bundled skills to ALL profiles automatically.",
    "evelyn gateway install sets up Evelyn as a system service (systemd/launchd).",
    "evelyn memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "evelyn webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: evelyn tools disables unused tools, evelyn skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "evelyn models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as evelyn -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Evelyn tools programmatically — results stay out of context.",
    "delegate_task defaults to 20 live runners globally, 5 per root session and Batch, with nesting depth 2; configure them under delegation.",
    ...
    "Profile names become shell commands — 'evelyn profile create coder' creates the 'coder' command.",
    "evelyn profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Evelyn prints a resume command with session ID and stats.",
    "evelyn sessions export backup.jsonl exports all sessions for backup or analysis.",
    "evelyn -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "evelyn skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: evelyn cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Evelyn runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "evelyn gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Evelyn via a local macOS server.",
    ...
    "Evelyn loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "evelyn mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "evelyn mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "evelyn mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — evelyn tools can toggle them per platform.",
    ...
    "evelyn chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "evelyn plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "evelyn acp runs Evelyn as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "evelyn auth add nous or evelyn auth add openai-codex sets up OAuth-based providers.",
    ...
    "evelyn status --deep runs deeper diagnostic checks across all components.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Evelyn your own.",
    ...
    "Before a gateway session reset, Evelyn auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope Evelyn state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'evelyn profile create ls' would be rejected.",
    "evelyn profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "evelyn claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via evelyn tools.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Evelyn auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Evelyn config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Evelyn is working.',
    ...
    # --- Curator ---
    'evelyn curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "evelyn curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'evelyn curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'evelyn auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'evelyn dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'evelyn dashboard embeds the full Evelyn TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'evelyn -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'evelyn chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'evelyn chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'evelyn chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "evelyn chat --source tool tags programmatic chats so they don't clutter evelyn sessions list.",
    'evelyn dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'evelyn sessions rename <ID> "new title" renames any past session; evelyn sessions delete <ID> removes one.',
    'evelyn import restores a session export or profile archive produced by sessions export or profile export.',
    'evelyn fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'evelyn pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'evelyn setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'evelyn status --deep runs the full health sweep across every component; plain evelyn status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Evelyn block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from evelyn sessions list by default — set --source explicitly to see them.',
