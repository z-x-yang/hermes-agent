...
def run_doctor(args):
    ...
    print(color("│                 🩺 Evelyn Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Evelyn a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Evelyn Persona\n\n"
                "<!-- Edit this file to customize how Evelyn communicates. -->\n\n"
                "You are Evelyn, a personal AI agent powered by Hermes Agent.\n",
                encoding="utf-8",
            )
