...
`Explore`, `Plan`, and `Reviewer` are one-shot profiles. Every child receives a short runtime-owned Core Contract plus explicit task data; complete active-profile `SOUL.md`/`MEMORY.md`/`USER.md` is not injected. `general-purpose` and `Reviewer` additionally receive real repository context and a workspace/git snapshot; successful GP work is automatically retained only when the parent session ID is nonempty and capacity is available. Reviewer accepts an ordinary self-contained prompt, completes with an ordinary final response, and exposes read/search/terminal plus readonly web only when the parent currently has web authority. Raw terminal means its no-edit/private-source contract is not a mechanical sandbox, and Reviewer still uses the ordinary operator-configured `delegation.max_iterations`.
