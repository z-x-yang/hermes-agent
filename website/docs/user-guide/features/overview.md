...
- **[Subagent Delegation](delegation.md)** — The `delegate_task` tool spawns child agent instances with isolated context, restricted toolsets, and their own terminal sessions. By default, Hermes permits 20 live runners process-wide and 5 per root session or Batch, with nesting depth 2.
