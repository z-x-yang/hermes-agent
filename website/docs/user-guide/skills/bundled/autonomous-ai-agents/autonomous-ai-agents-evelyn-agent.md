...
- **Context:** children receive a runtime Core Contract plus explicit task data and no parent history. Every GP additionally inherits the parent's SOUL/MEMORY/USER context regardless of provider, endpoint, ACP transport, or fallback. Explore, Plan, and Reviewer remain lean. GP and Reviewer additionally load repository project context/workspace snapshot. Reviewer requires `read_file`, `search_files`, and `terminal`; readonly web is optional under current parent authority. It has no named Notion, Mail, memory/session, browser, write/patch, process/execute-code, or delegation tools. Raw terminal is not a hard no-write/no-external-side-effect sandbox, so its prompt forbids mutation/private-source access and the controller verifies the worktree afterward.
