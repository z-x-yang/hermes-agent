...
| `DELEGATION_MAX_CONCURRENT_CHILDREN` | 每个 root session 的活跃 subagent runner 上限，同时也是单个 `delegate_task` Batch 的最大宽度（默认：`5`，下限为 1，无上限）。嵌套后代和 continuation 计入同一个 root session。也可通过 `delegation.max_concurrent_children` 配置——config 值优先。 |
