...
| [`evelyn-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-evelyn-agent) | 配置、扩展或贡献 Hermes Agent。 | `autonomous-ai-agents/hermes-agent` |
| [`independent-review-governance`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-independent-review-governance) | 在启动任何跨代码、研究、文档或制品的独立 Reviewer、Codex、Claude、人工或领域审查前使用；它是 review 授权、全局 per-change pass 计数、独立性边界、follow-up gate 与停止条件的唯一 owner，domain Skill 只能执行已授权 pass。 | `autonomous-ai-agents/independent-review-governance` |
...
| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`evelyn-agent-skill-authoring`](/user-guide/skills/bundled/software-development/software-development-evelyn-agent-skill-authoring) | 编写仓库内 SKILL.md：frontmatter、验证器、结构规范。 | `software-development/evelyn-agent-skill-authoring` |
...
| [`code-review-execution`](/user-guide/skills/bundled/software-development/software-development-code-review-execution) | 仅在 `independent-review-governance` 已授权审查、且已完成的软件改动涉及 shared/core、认证安全、并发、验证、不可逆动作或公共契约风险时使用。 | `software-development/code-review-execution` |
