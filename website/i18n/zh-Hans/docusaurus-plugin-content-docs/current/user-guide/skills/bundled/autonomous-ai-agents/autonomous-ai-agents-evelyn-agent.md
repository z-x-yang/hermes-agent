...
- **生命周期：** Explore/Plan/Reviewer 是一次性任务；Plan 只返回 implementation-plan 调研输入，最终 plan 由 parent 形成；成功的 general-purpose 在 parent session 与容量可用时自动保留。
- **续聊：** 同一 retained GP 工作使用 `delegate_continue(agent_id, prompt, run_in_background=...)`。
- **上下文：** child 获得 Core Contract 与显式 task data，不继承 parent history。同 provider/endpoint 的 GP 额外继承 parent 的 SOUL/MEMORY/USER context；跨 provider/endpoint route 或 fallback 会剥离它。Explore、Plan、Reviewer 保持 lean。GP 和 Reviewer 额外加载 repo project context/workspace snapshot。Reviewer 接受普通自包含 prompt，以普通 final response 完成；其 profile system instructions 固定预加载 scope/intent、call/data/error/cleanup path、runtime/test wiring、false-green 和 defect-relevant reuse/simplification/performance 审查方法，不开放动态 skill discovery。Reviewer 必须有 `read_file`、`search_files`、`terminal`，并只在 parent 当前拥有 web authority 时附加 readonly web。它不暴露 named Notion、Mail、memory/session、browser、write/patch、process/execute-code 或 delegation tools。Raw terminal 不是 hard no-write/no-external-side-effect sandbox，因此 prompt 禁止 mutation/private-source access，controller 在审查后检查 worktree。
