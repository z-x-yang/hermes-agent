...
- **上下文：** child 获得 lean Core Contract 与显式 task data，不注入完整个人 governance 或 parent history。GP 和 Reviewer 额外加载 repo project context/workspace snapshot。Reviewer 接受普通自包含 prompt，以普通 final response 完成，必须有 `read_file`、`search_files`、`terminal`，并只在 parent 当前拥有 web authority 时附加 readonly web。它不暴露 named Notion、Mail、memory/session、browser、write/patch、process/execute-code 或 delegation tools。Raw terminal 不是 hard no-write/no-external-side-effect sandbox，因此 prompt 禁止 mutation/private-source access，controller 在审查后检查 worktree。
