...
| 辅助任务（视觉、压缩等） | ✔（`provider: auto` 会先试每任务备用链，再试主备用链，最后才走内置辅助发现；显式辅助提供商遇到容量错误时也会使用主备用链） |
...
当任务的提供商设置为 `"auto"`（默认值）时，Hermes 会先尝试主提供商 + 主模型；如果该路径不可用或后续遇到容量类错误，会先遵循用户配置的备用策略，再使用内置发现链：
...
任务专属 `fallback_chain` 最精确，会优先使用。顶层 `fallback_providers` 是主 Agent 使用的同一套策略，因此 `provider: auto` 的辅助任务，以及显式辅助提供商在遇到容量错误时，都会遵守这条备用链。
...
以上每个任务均遵循相同的 **provider / model / base_url** 模式。每个任务也可以声明自己的 `fallback_chain`；如果省略，容量错误会使用顶层 `fallback_providers` 链。对于 `provider: auto`，Hermes 随后继续尝试内置辅助发现链；对于显式提供商，Hermes 会把主 Agent 模型作为最后安全网。
...
当你设置了显式的辅助提供商（例如 `auxiliary.vision.provider: glm`）时，Hermes 将其视为首选——但若该提供商因**容量错误**（HTTP 402 付款要求、HTTP 429 速率/用量/配额耗尽、连接失败）而无法处理请求，Hermes 会通过分层链进行备用，而不是静默失败：

1. **主辅助提供商** — 你配置的那个（始终优先尝试）
2. **`auxiliary.<task>.fallback_chain`** — 你的每任务覆盖列表（若已配置）
3. **顶层 `fallback_providers` / `fallback_model`** — 主 Agent 使用的同一套备用策略
4. **主 Agent 提供商 + 模型** — 最后的安全网（始终尝试，即使未配置链）
5. **警告 + 重新抛出** — 若所有层均失败，Hermes 以 WARNING 级别记录 `Auxiliary <task>: ... all fallbacks exhausted` 并重新抛出原始错误

本地同提供商恢复路径仍无法处理的 HTTP 429 会被视为容量问题，包括 SDK 只暴露错误字符串的 `usage_limit_reached`。它们会绕过显式提供商限制，让压缩、网页提取等辅助任务继续按你的备用策略运行。

对于使用 `provider: auto`（无显式辅助提供商）的用户，现有的自动检测链会替代最后的主 Agent 安全网。其第一步已经是主 Agent 模型，因此 `auto` 用户无需任何配置即可获得相同效果。

### 可选：每任务备用链

若你希望在顶层备用链之前使用不同的任务级备用顺序，可显式配置 `fallback_chain`。每个条目至少需要 `provider`；`model`、`base_url` 和 `api_key` 为可选。
...
你**不需要**配置 `fallback_chain` 才能获得备用功能——顶层 `fallback_providers` 和主 Agent 安全网都会运行。仅当你明确希望使用与默认不同的任务级顺序时才需配置。
...
- 通用：`daily quota`、`quota_exceeded`、`usage_limit_reached`
