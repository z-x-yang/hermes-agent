...
| `Plan` | 为 parent-owned implementation planning 提供只读调研输入 | 一次性；lean Core Contract + task data；不注入个人/项目上下文，也不产出最终 plan |
| `Reviewer` | 对 scoped code change 做 fresh-context 独立审查 | 一次性；默认前台；repo context + 普通自包含 prompt；普通 final response |
| `general-purpose` | 多步骤执行、编辑、测试和获准的外部动作 | 成功完成后自动保留；Core Contract 加项目上下文/workspace snapshot；所有 provider/endpoint/ACP/fallback route 都继承 parent 的 SOUL/MEMORY/USER context |

与 Claude Code 一样，每个 canonical profile 自己持有 routing `description`，告诉 parent 何时选择它。`Reviewer` 与其他 profile 使用同一个普通 `prompt` 字段，没有隐藏 JSON grammar 或 profile-specific completion tool。它的 profile system instructions 固定预加载一套 runtime-owned 审查方法：先确定 scope 与 intended behavior，再检查 callers/callees、数据边界、错误和清理路径，追踪 runtime wiring 与 false-green tests；reuse、simplification 和 performance 只有暴露具体缺陷时才升级为 finding。这是固定 profile 文本，不是动态 skill discovery，也不会自动继承其他产品的 bundled review command。`review_root` 是 Hermes 保留的顶层适配：当前 workspace 不是目标时，用它绑定一个本机 worktree。
