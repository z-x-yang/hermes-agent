...
| `general-purpose` | 需要编辑、测试的多步骤仓库工作 | 使用封闭白名单，可操作文件、原始 shell/进程、任务、技能和视觉工具；命名的消息、Notion、cron、记忆和委派工具均被排除，但 shell 并不是无副作用沙箱 | 后台 |

`Explore` 和 `Plan` 不能写文件、运行 shell、产生外部副作用或委派。`general-purpose` 可以编辑和测试，默认不能直接调用被排除的命名副作用工具，也不能继续委派。但它会保留原始 `terminal` 和 `process`：shell 命令可以访问网络、调用已认证 CLI 并产生外部副作用。这些行为仍受正常 terminal 审批和任务指令约束；Hermes 不为此 profile 提供硬性的无副作用隔离。
