...
| `Reviewer` | 对一个 frozen code target 做 fresh-context 独立审查 | 一次性；默认前台；固定 review bundle + 严格 review capsule；结构化结果必须通过校验 |
| `general-purpose` | 多步骤执行、编辑、测试和获准的外部动作 | 成功完成后自动保留；Core Contract 加项目上下文和 workspace snapshot |

与 Claude Code 一样，每个 canonical profile 自己持有 routing `description`，告诉 parent 何时选择它。若某个 profile 有特殊调用形状，还可以持有面向 parent 的 invocation guidance；runtime 会把它追加到 model-visible `subagent_type` schema。`Reviewer` 用这条路径说明严格 capsule 与 `review_root` 规则，而 backend validation 仍是最终 enforcement boundary。

省略 `subagent_type` 时解析为 `general-purpose`。child 不继承 parent transcript、parent tool result，也不自动注入 active profile 完整的 `SOUL.md`、`MEMORY.md`、`USER.md`。运行时 capability policy 仍是 trusted；除 `general-purpose` 自动加载真实 workspace project context 外，private/project facts 只能由 controller 明确放入 task capsule。

`Explore` 和 `Plan` 使用运行时强制的只读工具上限：可使用获准的 repo/file reader、no-spill web/skill reader、显式 `mode=readonly` 的 Notion AI，以及选定的 Apple Mail 读取工具；不给 raw terminal 和文件写入。`Reviewer` 只有 root-scoped review read/search、结构化只读 Git、按 capsule gate 的 readonly web 和 `report_review_findings`；没有 Notion、Mail、session/memory、MCP、browser、raw shell、write 或 delegation。`general-purpose` 只能获得通过 current parent 精确工具权限与正常安全合同共同筛选的工具；它不是“无外部副作用”的 sandbox。

`Explore`、`Plan` 和 `general-purpose` 的结果合同由 prompt 指定。`Reviewer` 是例外：free text 不能完成审查，必须通过 `report_review_findings` 提交经校验的 findings、correctness、confidence、短 file/line location、evidence 和 traced sources。Reviewer 只给 candidate blocker；parent/controller 必须逐条复现或证伪。
...
`Reviewer` 的 `prompt` 必须是一个严格 JSON object，不能是普通 prose：

```python
capsule = json.dumps({
    "original_ask_or_approved_contract": "修复 auth race，不改变 public API。",
    "acceptance_criteria_and_invariants": ["同一个 token 不得并发 refresh 两次。"],
    "relevant_repo_rules": ["Python 3.11；pytest 是 canonical runner。"],
...
`review_root` 是 controller 绑定的 tool argument，不属于 child capsule。它只允许用于顶层单个 Reviewer，且必须精确解析为现存的本机 absolute Git worktree root；relative path、repo 子目录、Batch/nested、非 Reviewer 和 remote/cluster root 都 fail closed。省略时审查当前 workspace。

Target mode 可为 `uncommitted`、`base`（必须提供 `base`）或 `commit`（必须提供 `commit`）。审查期间 scoped target 漂移会使结果失效。`external_reference_scope="none"` 会在 backend 前拒绝 web；`authoritative_docs_only` 只开放 readonly/no-spill web alias。
...
`general-purpose` 会加载真实 repo rules（按正常发现规则选择 `.hermes.md`、`AGENTS.md`、`CLAUDE.md` 或 `.cursorrules`）以及 workspace/git snapshot。`Explore`、`Plan` 和 `Reviewer` 都跳过自动 project context 与完整个人 governance 注入；Reviewer 只获得固定版本 bundle、最小 workspace identity 与 frozen capsule。
