...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. Evelyn 首选名；Hermes 兼容名（向上至 git 根）
...
| 优先级 | 文件 | 搜索范围 | 说明 |
|--------|------|----------|------|
| 1 | `.evelyn.md`、`EVELYN.md`；再到 `.hermes.md`、`HERMES.md` | 从 CWD 向上至 git 根目录 | Evelyn 原生配置并兼容 Hermes |
...
1. `.evelyn.md` / `EVELYN.md`，再到兼容的 `.hermes.md` / `HERMES.md`（向上遍历至 git 根目录）
...
- `$EVELYN_HOME/SOUL.md` — 用自定义 agent 角色和固定行为替换内置默认身份块。全新安装使用 `~/.evelyn`；已有 `~/.hermes` 继续兼容。
- `$EVELYN_HOME/MEMORY.md` 和 `$EVELYN_HOME/USER.md` — 提供应在新会话中快照的持久跨会话事实和用户配置文件数据（`HERMES_HOME` 保留为兼容别名）。
- 项目上下文文件，如 `.evelyn.md`、`EVELYN.md`、兼容的 `.hermes.md` / `HERMES.md`、`AGENTS.md`、`CLAUDE.md` 或 `.cursorrules` — 注入仓库特定的工作规则。
